            ...
            if (playerInGame) {
                if (playerInGame.words === null) {
                    var conditions = {
                        'gameId' : game.gameId,
                        'player' : player};
                    var update = {$set : {'words' : playerWords}};
        ...
        .exec(function handleGetAllPlayersInGameResult(err, players) {
            if (err) {
                ...
                if (foundNull) {
                    ...
                    createAndSendResult(player, players, game, res);
                }
            }
        });
}
    
function createAndSendResult(player, players, game, res) {
    console.log("Creating solution for player " + player + ", players " 
                + players + ", gameId " + game.gameId);
...
//            }
//            
//            var correctWords = playerInGame.words
//                .filter(function(word) {
//                    return solution.words.indexOf(word) >= 0;
//                });
//            var wrongWords = playerInGame.words
//                .filter(function(word) {
//                    return solution.words.indexOf(word) < 0;
//                });
//                
...
//            console.log('Entered words: ' + playerInGame.words);
